...
class MemoryStore:
    ...
    async def commit_messages(
        ...
        async with async_session_factory() as session:
            ...
            if ltm_message:
                # 檢查重複
                last_record = await hist_dao.get_last_by_step(
                    self._session_db_id, step_id
                )
                if (
                    not last_record
                    or last_record.msg_type != "system"
                    or last_record.content != ltm_message.content
                ):
            ...
            if (
                not last_record
                or last_record.msg_type != last_msg_type
                or last_record.content != last_content
            ):
    ...
    def _classify_response(self, response: BaseMessage) -> tuple[str, str]:
        """分類 Response 消息類型。

        Returns:
            (msg_type, sender)
        """
        # 檢查係咪 ToolMessage（Tool Result）
        from langchain_core.messages import ToolMessage

        if isinstance(response, ToolMessage):
            return ("tool_result", "System")
